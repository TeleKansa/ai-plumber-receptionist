...
## OPEN
- A-008 — Deploy shoreline vertical + selection wiring: merge `change/shoreline-vertical` → main (adds verticals/shoreline.json + per-tenant vertical selection). Built + verified @ dea2f5b: plumber byte-identical (golden), leakage clean, 148/148; default/legacy tenants unchanged (D-020/D-021). Effect: redeploys selection logic + the config file, but NO tenant uses shoreline yet → live behavior unchanged until a shorelinecost tenant + number exist. Requires a plumber-line regression test call after deploy. NOTE: does NOT make shoreline answer yet (step 3 lead delivery + provisioning + A-006 still pending). **Recommend: hold and batch with step 3 so shoreline ships in one deploy** (the branch is deploy-inert meanwhile).
